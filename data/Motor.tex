...
如果电动机的目标速度过高，电动机可能不会旋转。 确保电动机可以支持目标速度或通过加速以使电动机达到最高速度。

对于所需的电动机速度（v），微步级别（$n_m$）和电动机全步距角（$\theta_{\text {step }}$，一整步对应的角度），
...
较高的微步步进级别将意味着电动机运动更平稳且噪声较小，但会增加开关损耗，并且需要较高的$f_step$才能达到相同的电动机速度。
