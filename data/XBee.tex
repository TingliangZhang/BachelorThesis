...
% 如果在命令模式超时内未收到有效的AT命令，则无线模块将自动退出AT命令模式。
...
% API（Application Programming Interface 应用程序编程接口）操作模式是AT模式的替代。
...
% 要转义数据字节，请插入0x7D，然后在其后跟要转义的字节与0x20进行XOR。
...
% 如表~\ref{tab:XBee-Transmit-data-frames}。
...
% 如表~\ref{tab:XBee-Receive-data-frames}。
...
% 例程源码如附录~\ref{sec:NetworkModificationsSample}所示。
...
\mintinline{python}{send_data(RemoteXBeeDevice, String or Bytearray, Integer)}方法(Method)可以实现同步通信(Synchronous operation)，即等待发送完接收到Success confirm信息之后再运行其他程序，保证了每一条信息都送达，但可能回影响其他程序的持续运行。

% 例程源码如附录~\ref{sec:SendDataSample}所示。

\mintinline{python}{send_data_async(RemoteXBeeDevice, String or Bytearray, Integer)}方法(Method)可以实现异步通信(Asynchronous operation)，不会阻断其他程序的运行，但不能保证每一条都到达目标节点。

% 例程源码如附录~\ref{sec:SendDataAsyncSample}所示。

\mintinline{python}{send_data_broadcast(String or Bytearray, Integer)}方法(Method)可以实现广播(Broadcast transmissions)，即向网络中所有其他节点广播信息。

% 例程源码如附录~\ref{sec:SendBroadcastDataSample}所示。
